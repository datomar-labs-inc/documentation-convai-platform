...
2. Dialogflow returns the phone number in the ```phone-number``` parameter which can then be extracted using ```{{ d "phone-number" }}```
<img alt="Merge Users node explained" src={useBaseUrl('img/nodes/stock/merge-users/1.png')} />
