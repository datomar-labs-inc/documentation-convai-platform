...
Go to the Dev Console on the Right Side Panel (we will refer to it as Side Panel for short). Type “Ask a film question” and hit Enter. Click on View under the bot’s response, scroll down a little bit to inspect the Session Data and the Entity we got from DialogFlow, that is what the Script Node did for us.



If the AskQuestion Intent did not match, session data will look like this

We have the Get Question Module Ready, let’s add another Module named “Ask Question” on the Main Graph, connect it after the Get Question Module. Double click to enter its graph. 
...
Go to Dev Console in the Side Panel, send “Ask a film question” to the bot. The Bot will respond with the quiz question and you can play with it.
...
On the Main Graph, create a module named “Challenge” and connect it to Any Time System Node with Challenge intent Link, enter its graph.
...
We have the Challenge Module ready, now we need a Get Point Module for the user to get its score or reset it. Create this Module on the Main graph and connect it to Any Time system Node with GetPoints intent link.
...
Here is just a demonstration of the welcome message, you can add more information to optimize user experience.
The final graph of the enhanced Tutorial Bot:

The overall execution of the enhanced Tutorial Bot:

That’s it! You have learned how to create a bot with NLP and External APIs, and how to use Modules to make your development process flexible. Now let’s proceed to integrate the bot into Facebook

## Facebook Channel
In this section, we will integrate the bot into Facebook so that you can chat to the bot on Facebook.
In Convai, go to Settings via Side Nav. Add Facebook integration, leave the configurations blank and save it. For now you have Facebook and DialogFlow integrations.

Login to Facebook for Developer. Click on My Apps
...
On the left side panel, click on the + button on the right of PRODUCTS.

Find Messenger and click Set Up.

Go to Settings -> Basic, copy the App Secret into Convai Facebook Integration Configuration


On Facebook for Developer, go back to Messenger Settings
...
Next you need to Add Subscription for the webhook we added in the previous step.
...
Login to Dashbot, click on Add Chatbot.

Fill in the bot’s name and select “Universal” as the Dashbot platform. Click on Register.

Copy the API key into Convai’s integration configuration and Save it.



Back to the bot’s graph. Go to Extension Graph via Paginator.
Add a Dashbot Node by Context Menu

In Dashbot under the Integration Code Section, click View Reports to go to the Dashboard.


Back to Convai. Do some simple execution in the Dev Console.
...
Create a new Module named “Fallback”. Connect it to the Any Time System Node with a basic link - user input is anything other than the intents we have on other modules and nodes. 
...
Using Fallback optimizes User Experience when the user input something wrong. And it is the bot developer’s responsibility to let users know how to do things right. That means we need to tell the user how to do things correctly in ANY Response Node that prompts a user input.
...
The overall execution of the User Experience Optimized bot

Congratulations, you have learned all of the basics you need to develop a chatbot in Convai. You can always check back here if you have any questions. It is good for you to start developing your own bot and check out other detailed documentations of Convai.
